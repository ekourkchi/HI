...
pro mymwrfits, xinput, file, header,           $
    ...
    if (keyword_set(Version)) then begin
        print, "MWRFITS V"+mwr_version()+":  March 2, 2007"
    endif

    if n_elements(file) eq 0 then begin
        if (not keyword_set(Version)) then begin
            print, 'MYMWRFITS: Usage:'
            print, '    MYMWRFITS, struct_name, file, [header,] '
           ...
           print, '             /USE_COLNUM, ALIAS=, /COMPRESS'
        endif
        return
    endif
  ...
  not_found:
	...
	if strlowcase(strmid(file,2,3,/reverse)) NE '.gz' $
	   then ext = '.gz'
    endif else compress = 0
    on_ioerror, open_error
    openw, lun, file + ext, /get_lun, /swap_if_little, compress=compress
    bof = 1
    on_ioerror, null
