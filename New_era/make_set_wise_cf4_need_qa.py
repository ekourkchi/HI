...
if __name__=='__main__':
    ...
    destin = '/run/media/ehsan/6ccd3c78-12e8-4f00-815d-faf200b314cf/ehsan/db_esn/'+'/cf4_wise_set03/data/'
    
    pgcname, radb = read_glga('set03_wise_cf4_need_qa.glga')
    ...
    for i in range(len(pgcname)):
        ...
        for filter in ['w1','w2','w3','w4']:
            ...
            desti = destin+radb[i]+'/wise/fits/.'
            if os.path.exists(fname):
                command = ["cp", fname, desti]
                print command
        ...
        for filter in ['w1','w2','w3','w4', 'w123', 'w124']:
            ...
            desti = destin+radb[i]+'/wise/jpg/.'
            if os.path.exists(fname):
                command = ["cp", fname, desti]
                print command
                subprocess.call(command)        
        
        fname = databse+radb[i]+'/plots/'+pgcname[i]+'*'
        desti = destin+radb[i]+'/plots/.'
        cmd = "cp "+ fname+' '+ desti
        xcmd(cmd,True)  
        
        fname = databse+radb[i]+'/photometry/'+pgcname[i]+'*dat'
        desti = destin+radb[i]+'/photometry/.'
        cmd = "cp "+ fname+' '+ desti
        xcmd(cmd,True)           
    

        fname = databse+radb[i]+'/aux/'+pgcname[i]+'_ellipse.dat'
        desti = destin+radb[i]+'/aux/.'
        cmd = "cp "+ fname+' '+ desti
        xcmd(cmd,True)           
