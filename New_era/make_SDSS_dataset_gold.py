...
if __name__ == '__main__':
  ...
  inFile  = 'IMPORTANT_ehsan_NOTES'
  table   = np.genfromtxt(inFile , delimiter=',', filling_values=None, names=True, dtype=None)
  PGC     = table['PGC']
  
  
  db_dir = '/run/media/ehsan/6ccd3c78-12e8-4f00-815d-faf200b314cf/ehsan/db_esn/data_gold1/'
  print '# This is the glga qa file ....'
  with open('data_gold1.glga', 'w') as file:
     for j in range(len(PGC)):
        i = 0
        while i<N and pgc[i] != int(PGC[j][3:]): i+=1   
        
        if SDSS_db_avilability(pgc[i], ra[i], dec[i], Filters=['g','r','i']) == 3:
           file.write('{0} {1} {2} {3} {4} {5} {6}\n'.format('pgc'+str(pgc[i]), ra[i], dec[i], d25[i], d25[i]*b_a[i], PA[i], Ty[i]))
