...
if __name__ == '__main__':
  ...
  count = 0
  db_dir = '/run/media/ehsan/6ccd3c78-12e8-4f00-815d-faf200b314cf/ehsan/db_esn/data_gold2/'
  print '# This is the glga qa file ....'
  with open('data_gold2.glga', 'w') as file:
    for j in range(len(pgc)):
         
      if not pgc[j] in pgc2 and sdss[j]==1:
                
        if SDSS_db_avilability(pgc[j], ra[j], dec[j], Filters=['g','r','i']) == 3:
           count+=1
           file.write('{0} {1} {2} {3} {4} {5} {6}\n'.format('pgc'+str(pgc[j]), ra[j], dec[j], d25[j], d25[j]*b_a[j], PA[j], Ty[j]))
           SDSS_db_avilability(pgc[j], ra[j], dec[j], db_dir=db_dir, toDbase=True)
           #print count
