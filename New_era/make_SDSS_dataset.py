...
def SDSS_db_avilability(pgc, ra, dec, Filters=['u', 'g', 'r', 'i', 'z'], db_dir='', toDbase=False):
   ...
   if True:
     ...
     for filters in Filters:
        ...
        fname = "/home/ehsan/db_esn/REMAINDER_SDSS/"+pgc_dir+'/'+pgc_dir+'_'+filters+'.fits.gz'
        fname2 = '/home/ehsan/db_esn/SDSS_ALL_Images/'+pgc_dir+'_'+filters+'.fits.gz'
	   ...
	   if toDbase:
	      ...
	      subprocess.call(['rm', desitination+pgc_dir+'*back'])   # zipping back
...
if __name__ == '__main__':
  ...
  ##############################################
  pilot_lst = []
  datasets = ['pilot_01.glga', 'pilot_02.glga', 'pilot_03.glga', 'pilot_05.glga']
  for dataset in datasets:
     pilot_lst += read_glga(dataset)  
  
  datasets = ['data_set_11.glga', 'data_set_21.glga', 'data_set_31.glga', 'data_set_41.glga', 'data_set_51.glga', 'data_set_12.glga', 'data_set_22.glga'] 
  ...
  dataummer = []
  for i in range(20):
      dataummer.append('data_summer_'+str(i+1)+'.glga')
  
  for dataset in dataummer:
     pilot_lst += read_glga(dataset)
  ...
  if True:
	...
	if True:
	    ...
	    for j in range(len(pgc)):
	      ...
	      if sdss[j] == 1 and SDSS_db_avilability(pgc[j], ra[j], dec[j], Filters=['g','r','i']) < 3 and not pgc[j] in selected_pgc and not pgc[j] in pilot_lst:
		selected_pgc.append(pgc[j])
		selected_ind.append(j)
		#p+=1
		print 'pgc'+str(pgc[j])
...
##############################################################3
  ...
  #print len(selected_pgc)
  #ext_HDD = '/run/media/ehsan/HOURI/DATA_SUMMER/'
  
  #print '# This is the glga qa file ....'
  
  #for q in range(4):
      #with open(ext_HDD+'data_summer_'+str(q+17)+'.glga', 'w') as file:
          #begin = q*300
          #end = (q+1)*300
          #for i in range(begin,end):
            #ind = selected_ind[i]
            #file.write('{0} {1} {2} {3} {4} {5} {6}\n'.format('pgc'+str(pgc[ind]), ra[ind], dec[ind], d25[ind], d25[ind]*b_a[ind], PA[ind], Ty[ind]))
  ...
  #for q in range(4):
      #begin = q*300
      #end = (q+1)*300
      #db_dir = 'data_summer_'+str(q+17)+'/'
      #for i in range(begin,end):
         #ind = selected_ind[i]
         #SDSS_db_avilability(pgc[ind], ra[ind], dec[ind], db_dir=ext_HDD+db_dir, toDbase=True)
