...
if __name__=='__main__':
    ...
    pgcname, radb = read_glga('cf4_sdss_all.glga')
    ...
    for i in range(len(pgcname)):
        ...
        with cd(databse+radb[i]+'/sdss/fits/'):
            
            #for filter in ['w1','w2','w3','w4']:
            for filter in ['u','g','r','i', 'z']:
                if os.path.exists(pgcname[i]+'_'+filter+'.fits.gz'):
                    if  os.path.exists(pgcname[i]+'_'+filter+'.fits'):
             
                        command = ["rm", pgcname[i]+'_'+filter+'.fits']
                        print command
