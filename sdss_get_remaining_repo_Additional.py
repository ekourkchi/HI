...
if __name__ == '__main__':
  ...
  repository = '/home/ehsan/db_esn/SDSS_Additional/' 
  
  
  inFile  = 'augment/EDD_distance_cf4_v06.csv'
  ...
  inFile  = 'New_era/IMPORTANT_ehsan_Additional'
  table   = np.genfromtxt(inFile , delimiter=',', filling_values=None, names=True, dtype=None)
  PGC     = table['PGC']
  ...
  #p=0
  #while p<len(PGC) and int(PGC[p][3:]) != 72115: p+=1
  ...
  for j in range(len(PGC)):
      ...
      i = 0
      while i<N and pgc[i] != int(PGC[j][3:]): i+=1 
      ...
      if not os.path.isdir(repository+'pgc'+str(pgc[i])):
