...
def fits_prep(filename, edit_zp=True):
  ...
  if edit_zp:
     ...
     ### nanomaggie: A linear unit of flux density equal to 10^-9 maggies. A star of brightness 1 nanomaggie has a magnitude of 22.5 in any band, or a flux density of 3.631 × 10^-6 Jansky.
     zeroPoint = 22.5
...
if __name__ == '__main__':
  ...
  repository = '/home/ehsan/db_esn/SDSS_GOLD/' 
  ...
  inFile = 'augment/helene_nancy.csv'
  table = np.genfromtxt(inFile , delimiter=',', filling_values=None, names=True, dtype=None)  
  ID = table['ID'] 
  ...
  #p=0
  #while p<len(PGC) and int(PGC[p][3:]) != 1977897: p+=1
  
  
  
  for j in range(len(ID)):
      
    if extract_pgc(ID[j]) in pgc: 
      i = 0
      while i<N and pgc[i] != extract_pgc(ID[j]): i+=1 
       

      
      if isInSDSS_DR12(ra[i], dec[i]) == 1 and not os.path.isdir(repository+'pgc'+str(pgc[i])) and not pgc[i] in pgc2:
              ...
              moderate_sdss(pgc[i], repository)
