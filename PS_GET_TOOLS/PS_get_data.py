...
######################################
######################################
def make_PS(db_dir, pgc, ra, dec, d25):
  ...
  for band in ['g','r','i']:
    ...
    #my_images =  glob.glob(pgc_band_root+'/tmp/*.unconv.mk.fits')
    #for i in range(len(my_images)):
      #with cd(pgc_band_root):
        #fits_prep(my_images[i], 8)        
...
############################################################################
    #with cd(pgc_band_root):
      ...
      #mImgtbl projected pimages.tbl
      #mAdd -p projected pimages.tbl tmp.hdr tmp.fits
      #mConvert -b 8 tmp.fits tmp.fits
      #rm *area*fits
      #rm *tbl
      #rm -rf projected
      #'''
      ...
      #xcmd('tcsh montage.csh', True)
      #xcmd('mv tmp.fits '+'../pgc'+str(pgc)+'_'+band+'.mask.fits', True)
############################################################################
...
db_dir = '/home/ehsan/db_esn/my_PS_db_2/'
##db_dir = '.'
...
row_min = sys.argv[1]
row_max = sys.argv[2]
#inFile = str(sys.argv[1])   # *.catal
inFile = 'alexandra_251.csv.'+row_min+'_'+row_max+'.catal'
db_dir = db_dir+'/'+row_min+'_'+row_max
if not os.path.isdir(db_dir):
  cmd = 'mkdir '+db_dir
  xcmd(cmd, True)

URL_root = str(sys.argv[3])+'/'
...
table = np.genfromtxt(index_file , delimiter='|', filling_values=None, names=True, dtype=None, skip_header=5)
fileID         = table['fileID']


#all.9060.csv
table = np.genfromtxt('alexandra_251.csv', delimiter=',', filling_values=None, names=True, dtype=None)
pgc     = table['PGC']
...
for i in range(len(fileID)):
  ...
  if True: #row_s>=1762:  # True
      pgc_no  = PGC[row_s-1]
      d25_no  = d25[row[row_s-1]-1]
      ra_no   = ra[row[row_s-1]-1]
      dec_no  = dec[row[row_s-1]-1]
      file_id = fileID[i]
      
      if COUNTER!=0 and pgc_previous!=pgc_no:
	## Making postage stampe image
	make_PS(db_dir, pgc_previous, ra_previous, dec_previous, d25_previous)
      ...
      dec_previous = dec_no 
      if band in ['g','r','i']:
	download_pgc_band(URL_root, file_id, db_dir, pgc_no, band)
	COUNTER+=1
