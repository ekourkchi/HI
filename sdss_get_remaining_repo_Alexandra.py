...
if __name__ == '__main__':
  ...
  repository = '/home/ehsan/db_esn/SDSS_Alexandra/' 
  ...
  inFile = 'augment/output_1_Results_alexandra.csv'
  table = np.genfromtxt(inFile , delimiter='|', filling_values=None, names=True, dtype=None)  
  id_lexi = table['ID'] 
  ...
  p=0
  while p<len(id_lexi) and int(id_lexi[p][3:10]) != 91783: p+=1
  
  
  for j in range(p, len(id_lexi)):
    
    pgc_lexi = int(id_lexi[j][3:10])
    if pgc_lexi in pgc: 
      i = 0
      while i<N and pgc[i] != pgc_lexi: i+=1 
       

      
      if pgc_lexi in pgc_sdss and not os.path.isdir(repository+'pgc'+str(pgc[i])) and not pgc_lexi in pgc2:
