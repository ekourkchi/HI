...
#inFile = 'cosmic_flows_sipitzer_EDD.csv'
#table = np.genfromtxt(inFile , delimiter='|', filling_values=None, names=True, dtype=None)
#pgc_cf2s    = table['PGC']

#p = 0 
#for i in range(len(pgc_cf2s)):
...
inFile = 'wise_calib_EDD.csv'
table = np.genfromtxt(inFile , delimiter='|', filling_values=None, names=True, dtype=None)
pgc_wise_calib    = table['PGC']

q = 0 
for i in range(len(pgc_wise_calib)):
    
    new_pgc = pgc_wise_calib[i]
    if not new_pgc in pgc_:
        pgc_add.append(new_pgc)
        q+=1
...
myTable.write('EDD_distance_cf4_v14.csv', format='ascii.fixed_width',delimiter='|', bookend=False) 
