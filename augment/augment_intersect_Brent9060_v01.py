    ...
    for j in range(len(pgc_leda)):
        if pgc_leda[j] == pgc_add[i]:
            if not pgc_add[i] in pgc_:
                ...
                alfalfa70.append(0) 
                if QA_SDSS_DONE(pgc_leda[j], ra_leda[j]):
                    QA.append(1) 
                else: QA.append(0) 

                if  QA_WISE_DONE(pgc_leda[j], ra_leda[j]):
                    QA_wise.append(1) 
                else: QA_wise.append(0)             
...
myTable.write('EDD_distance_cf4_v07.csv', format='ascii.fixed_width',delimiter='|', bookend=False) 
