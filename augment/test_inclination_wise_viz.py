...
for i in range(len(pgc_wise_vizier)):
    ...
    if pgc_wise_vizier[i] in pgc_incout:
        
        i_lst = np.where(pgc_incout == pgc_wise_vizier[i])
        if flag_incout[i_lst][0]<=0:

           my_inc.append(inc_incout[i_lst][0])
           pgc_common.append(pgc_wise_vizier[i])
           th_inc.append(inc_wise_vizier[i])
           
           #print pgc_wise_vizier[i]
...
ax.plot(th_inc, my_inc, 'r.', picker=5)
...
ax.set_xlabel('Inc. (Neil+14) [deg]', fontsize=14)
