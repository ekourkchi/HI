...
############################################################
QA_done = []
datasets = ['pilot_01.glga', 'pilot_02.glga', 'pilot_03.glga', 'pilot_05.glga', 'data_set_11.glga', 'data_set_12.glga', 'data_set_21.glga', 'data_set_22.glga', 'data_set_31.glga', 'data_set_41.glga', 'data_set_51.glga', 'data_summer_1.glga', 'data_summer_2.glga', 'data_summer_3.glga', 'data_summer_4.glga','data_summer_5.glga', 'data_summer_6.glga','data_summer_7.glga','data_summer_8.glga', 'data_summer_9.glga', 'data_summer_10.glga', 'data_summer_11.glga', 'data_summer_12.glga', 'data_summer_13.glga', 'data_summer_14.glga', 'data_summer_15.glga', 'data_summer_16.glga', 'data_summer_17.glga', 'data_summer_18.glga', 'data_summer_19.glga', 'data_summer_20.glga', 'data_gold1.glga', 'data_gold2.glga'] 
for dataset in datasets:
     QA_done += read_glga('../New_era/'+dataset)
     
############################################################
############################################################
WISE_QA_done = []
datasets = ['wise_needs_qa_cosmicflows_set01.glga', 'wise_qa_4.glga', 'wise_needs_qa_cosmicflows_set02.glga'] 
for dataset in datasets:
...
table = np.genfromtxt( inFile , delimiter=',', filling_values=None, names=True, dtype=None)
wise_name_need_qa = table['name']
wise_pgc_need_qa  = table['pgc']
...
datasets = ['data_gold3_wise_nancay.glga', 'data_gold4_wise_remaining.glga'] 
for dataset in datasets:
     WISE_QA_done += read_glga('../New_era/'+dataset)


for i in range(len(wise_pgc)):
    if not wise_pgc[i] in wise_pgc_need_qa and not wise_pgc[i] in WISE_QA_done:
        WISE_QA_done.append(wise_pgc[i])


print len(WISE_QA_done), len(QA_done)
...
#for i in range(len(pgc)):
   #for j in range(len(pgc_ex)):
       #if pgc[i] == pgc_ex[j]:
           #print pgc[i], d25[i], pgc_ex[j], 0.1*(10**lgd25_ex[j])
           #break
...
for i in range(len(pgc_ex)):
    if not pgc_ex[i] in pgc_:
        ...
        ty_.append(Ty_ex[i])
        alfalfa70.append(1) 
        if pgc_ex[i] in QA_done:
            QA.append(1) 
        else: QA.append(0) 

        if pgc_ex[i] in WISE_QA_done:
            QA_wise.append(1) 
        else: QA_wise.append(0) 
...
for i in range(len(pgc_nancy)):
    ...
    if not pgc_nancy[i] in pgc_:
                ...
                alfalfa70.append(0) 
                if pgc_nancy[i] in QA_done:
                    QA.append(1) 
                else: QA.append(0) 

                if pgc_nancy[i] in WISE_QA_done:
                    QA_wise.append(1) 
                else: QA_wise.append(0)   
    ...
    for j in range(len(pgc_leda)):
        if pgc_leda[j] == pgc_add[i]:
            if not pgc_add[i] in pgc_:
                ...
                alfalfa70.append(0) 
                if pgc_leda[j] in QA_done:
                    QA.append(1) 
                else: QA.append(0) 

                if pgc_leda[j] in WISE_QA_done:
                    QA_wise.append(1) 
                else: QA_wise.append(0)             
...
myTable.write('EDD_distance_cf4_v05.csv', format='ascii.fixed_width',delimiter='|', bookend=False) 
